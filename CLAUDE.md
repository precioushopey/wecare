...
### i18n — `src/i18n/`
- `config.ts` inits react-i18next: `de` default + fallback, `en` toggle. Namespaces: `common` (nav/footer/shared), `home`, `conditions`, `assessment`, `dashboard`, `shop`, `faq`, `costs`, `knowledge`. Add a new namespace rather than growing one. Language persists to `localStorage` (`wecare.language`) and syncs `<html lang>`.
...
**Content still stubbed**: `/about`, `/careers`, `/providers`, `/contact`, `/legal/*` are localized `PagePlaceholder`s (title + one-line description only). COA values, product data, auth backend, per-product FAQ are placeholders. **Knowledge Hub article *bodies*** are unwritten — the index/filter/template exist (`KnowledgeHubPage` / `KnowledgeArticlePage` / `src/data/knowledge.ts`, `knowledge` ns), each article page shows an "in preparation" state; a writer + medical review fills the bodies.
...
- "How WeCare works" homepage section sits below the final CTA; `/how-it-works` redirects to it.
- Homepage has a **7th section**, `ComparisonSection` (guided vs. catalog), between trust and the final CTA.
...
- `<MedicalNotice>` (`common:medicalNotice.*`) on every condition landing page and knowledge article.
- Knowledge Hub built (index + topic filter + `/knowledge-hub/:slug` template); bodies pending (see above).
- Hero image is static (no float).
