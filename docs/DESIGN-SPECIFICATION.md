...
| FR-014 | Checkout (**auth-gated**, and gated on an `approved` review + `COMMERCE_ENABLED`): signed-in email as a read-only confirmation + Austrian shipping address — **separate First name / Last name** (PO 2026-09-04), street, postcode, city, country (read-only), **optional phone** — with per-field `autocomplete`/`inputmode` and **inline blur validation** (adaptive `checkout.errors.*`, focus jumps to the first error) + payment method from `ENABLED_PAYMENT_METHODS` (invoice / bank transfer) + **Terms checkbox** (`<Trans>` links) + **required disclaimer checkbox**; "Place order" disabled until both ticked; delivery fee = €0; `CheckoutSteps` bar (Cart → Details → Review). While `!COMMERCE_ENABLED` the whole form is replaced by a `checkoutUnavailable.*` panel (no "Place order" on placeholder prices — PO hard blocker; `import.meta.env.DEV` bypasses locally). | Auth | Must | IMPLEMENTED (**no real payment; commerce gated**) | FR-013, FR-017 | `CheckoutPage`. `placed` flag exempts the empty-cart guard during submit (WC-01). |
