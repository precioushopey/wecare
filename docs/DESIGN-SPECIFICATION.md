...
| FR-014 | Checkout (**auth-gated**, and gated on an `approved` review + `COMMERCE_ENABLED`): signed-in email as a read-only confirmation + Austrian shipping address — **separate First name / Last name** (PO 2026-09-04), street, postcode, city, country (read-only), **optional phone** — with per-field `autocomplete`/`inputmode` and **inline blur validation** (adaptive `checkout.errors.*`, focus jumps to the first error) + payment method from `ENABLED_PAYMENT_METHODS` (invoice / bank transfer) + **Terms checkbox** (`<Trans>` links) + **required disclaimer checkbox**; "Place order" disabled until both ticked; delivery fee = €0; `CheckoutSteps` bar (Cart → Details → Review). While `!COMMERCE_ENABLED` (= `!PRICES_CONFIRMED`) the whole form is replaced by a `checkoutUnavailable.*` panel — no "Place order" on placeholder prices (PO hard blocker), in dev and production alike. | Auth | Must | IMPLEMENTED (**no real payment; commerce gated**) | FR-013, FR-017 | `CheckoutPage`. `placed` flag exempts the empty-cart guard during submit (WC-01). |
| FR-015 | Placing an order records a mock order (`WC-<base36 timestamp>`, lines, total, status, `paymentMethod`) and clears the cart; status = `inReview` (cart always has a prescription item). The **shipping address is stored in `sessionStorage`** (`wecare.order-addresses`), never `localStorage`, and merged onto the order on read (PO 2026-09-04 — real addresses must not persist to disk client-side). | System | Must | MOCK/PLACEHOLDER | FR-014 | `orders/orders.ts`: metadata in `localStorage:wecare.orders`, address in `sessionStorage`. `shipped`/`delivered` statuses exist but are never set by the app. |
| FR-016 | Order confirmation page resolves the full order by `state.orderId` (falls back to `getOrders()[0]` on refresh/direct visit, WC-01); redirects home if none. Shows: line items + thumbnails, order total, payment method, delivery address (+ phone), a **positive forward-looking body + 3-step status list**, a stage-based `timingNote` (**no invented delivery date** — PO 2026-09-04), a support-contact line, `CheckoutSteps current="complete"`, and a primary CTA **"View order status"** (not "Track your order" — no carrier tracking). | Auth | Must | IMPLEMENTED | FR-015 | `OrderConfirmationPage`. |
...
| FR-021 | Dashboard content widgets, each backed by a real record — never fabricated: a 4-stage **patient journey** stepper (`journeyState()` from `review.status` + latest order), a "Your recommendation" panel, a full six-answer assessment recap, a "delivery" card (before an order: `AustriaMap` with the 7 target-city pins + `deliveryBanner.coverageNote`; once an order exists: the shipping address + a real-order tracker via `DELIVERY_STAGE` — **no fabricated live van/GPS/ETA**), an Orders `<table>` at `lg+` (with shipping address + payment method), an editable Profile form. | Auth | Should | IMPLEMENTED | FR-020 | `dashboard/ui.tsx` (`SectionCard`, `DashboardJourney`), `dashboard/pages.tsx`. |
...
| FR-023 | Six `/legal/*` documents with **draft** sectioned content, a TOC when a doc has > 4 sections. **Effective dates are blank** → the page shows "Draft — not yet in effect" until real dates are stamped at go-live (PO 2026-09-04). The Impressum's "Service provider" block + phone were **removed** (fabricated entity data); `imprint` + `privacy` show a `preLaunchNotice` while `!seoIndexable()`. Shipping's "Delivery times" section is neutralised + carries a `[LEGAL REVIEW REQUIRED]` marker. | All | Must | PARTIAL (draft, unreviewed; real entity data + counsel review = launch blocker) | — | `LegalPage`, `legal` namespace. |
...
```mermaid
flowchart TD
    A[Solution page: review approved AND COMMERCE_ENABLED?] -->|No| A0[Show orderingDisabledNote / Continue to medical review]
    A -->|Yes| A1[Pick grams 5/10/15/30]
    A1 --> B[Add to cart → navigate to cart]
    B --> C[/dashboard/cart/]
    C -->|Empty| Z[Empty state → View recommendation]
    C -->|COMMERCE_ENABLED=false| CG[Final-price note, no checkout CTA]
    C --> D[Adjust qty ±5g / remove]
    D --> E[Checkout]
    E --> F0{Authenticated?}
    F0 -->|No| LOGIN[/login reason=checkout → back]
    F0 -->|Yes| RA{Review approved?}
    RA -->|No| RR[Redirect to /assessment/medical-review or /review]
    RA -->|Yes| CE{COMMERCE_ENABLED?}
    CE -->|No| CU[checkoutUnavailable panel — no form]
    CE -->|Yes| F{Cart empty?}
    F -->|Yes| C
    F -->|No| G[Fill first/last name + shipping + optional phone + payment method; inline validation]
    G --> H{Terms AND disclaimer checked?}
    H -->|No| I[Place order disabled]
    H -->|Yes| J[Place order → addOrder status=inReview + shipTo→sessionStorage + paymentMethod, clear cart, placed=true]
...
6. **`DeliveryBannerSection`** — a gradient band: "Delivery across Austria." + generic body ("everywhere in Austria… once your prescription is issued") + an `AustriaMap` with pins on the 7 target cities (owner override 2026-09-04 — targets, not verified coverage) + `coverageNote` ("Availability depends on medical and pharmacy partner coverage"). No SLA / cutoff.
...
| F-34 | `PRICES_CONFIRMED` / `COA_CONFIRMED` / **`COMMERCE_ENABLED`** gating flags | System | Never present fabricated data as real; no "Place order" on placeholder prices | Must | IMPLEMENTED | — | FR-039 | US-008/010 |
...
| F-36 | Dashboard delivery card: shipping address + real-order tracker (pre-order: `AustriaMap` target-city pins + coverage note) | Auth | Delivery expectations without fabricating a live position | Should | IMPLEMENTED | F-15 | FR-021 | US-011 |
...
| BR-043 | The 6 `/legal/*` documents are **draft** text. **No fabricated entity data** (PO 2026-09-04): the Impressum "Service provider" block + phone are removed; the Privacy Policy controller/DPO block is reduced to the real support email + a "stated once confirmed" line; **effective dates are blank** and the page shows "Draft — not yet in effect"; `imprint` + `privacy` carry a `preLaunchNotice` while `!seoIndexable()`. **Not reviewed legal advice** — a real registered-entity identification block + Austrian/EU counsel review are hard launch blockers. |
...
| BE-05 | Pricing + review fee | Solution pages, cart, checkout, `/costs` | Real prices **at the `DispensingOption` level** (not the Solution), VAT handling, the deliberately-unstated **review fee** (D5), delivery policy (currently €0, D8). Then flip `PRICES_CONFIRMED` — which also lifts `COMMERCE_ENABLED` and re-opens checkout (disabled in prod until then, PO 2026-09-04). |
...
| BE-07 | Checkout + payment | `/dashboard/checkout` "Place order" | Payment **after** an `approved` review for MVP; invoice / bank transfer only (D7). The abstraction seam exists — `src/features/payments/payments.ts` (`PAYMENT_METHODS` with `enabled`, `requestPayment()` no-op); wire a PSP branch here to add card / SEPA / Klarna. Server-side address record (the client keeps it in `sessionStorage` only). Address validation. Record the two required confirmation checkboxes as consent/audit. |
| BE-08 | Orders + fulfilment/shipment tracking | `/shop/confirmation`, `/dashboard/orders`, dashboard delivery card | `GET /orders/me`, `GET /orders/:id`. Real statuses incl. `shipped`/`delivered` + carrier/tracking (the dashboard delivery tracker says outright it's placeholder until courier tracking is connected). **Copy-integrity:** the order-confirmation page already promises the user "an email at each step" (`shop:confirmation.body`) — so an email on every order-status transition (received → in review → dispatched → delivered) is **required, not optional**, or the shipped copy is a broken promise. Fire via BE-12. |
...
| BE-12 | Transactional email / notifications (DE/EN) | review status, order status, follow-up reminder, account verification | EU-compatible provider + templates. **Not a nice-to-have:** the review-status page ("We'll email you as soon as the status changes", `assessment:review.reassure`) and the order-confirmation page ("an email at each step") already tell the user these emails will arrive — the real build must send them (review transitions per BE-03, order transitions per BE-08) or the live copy is misleading. |
| BE-13 | Legal entity data | `/legal/*` (Impressum block + phone currently **removed**; effective dates **blank**) | Restore a real, counsel-reviewed Impressum/Firmenbuch/VAT/DPO/address block; stamp real effective dates at go-live; then counsel review of the full text (D9). Hard launch blocker. |
