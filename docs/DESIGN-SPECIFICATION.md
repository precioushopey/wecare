...
6. **`DeliveryBannerSection`** — a gradient band: "Delivery across Austria." + generic body ("everywhere in Austria… once your prescription is issued") + a neutral `AustriaMap` outline (**no city pins**) + `coverageNote` ("Availability depends on medical and pharmacy partner coverage"). No SLA / cutoff / city list (PO 2026-09-04).
...
| F-36 | Dashboard delivery card: shipping address + real-order tracker (pre-order: neutral `AustriaMap` outline + coverage note) | Auth | Delivery expectations without fabricating a live position | Should | IMPLEMENTED | F-15 | FR-021 | US-011 |
...
| `OrbitRings` / `AustriaMap` | `components/marketing/` | Homepage anchor-photo rings / **neutral Austria outline** (no city pins — PO 2026-09-04) | `AustriaMap` also reused in the dashboard delivery card (pre-order only). |
